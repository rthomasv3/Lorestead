...
Both variables are required - the server refuses to start without them. `LORESTEAD_TOKEN` is the bearer token every device and agent authenticates with. `LORESTEAD_DB_KEY` encrypts the server database at rest, so a copy of the volume is not a copy of your notes.
