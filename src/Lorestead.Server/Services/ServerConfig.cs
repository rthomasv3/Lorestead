...
public sealed class ServerConfig
{
    ...
    {
        string token = Environment.GetEnvironmentVariable("LORESTEAD_TOKEN");

        if (string.IsNullOrWhiteSpace(token))
        {
            throw new InvalidOperationException("LORESTEAD_TOKEN environment variable is required.");
        }
        ...
        {
            throw new InvalidOperationException("LORESTEAD_DB_KEY environment variable is required.");
        }

        string dataDir = Environment.GetEnvironmentVariable("LORESTEAD_DATA_DIR");
        ...
        string dbFilePath = Path.Combine(dataDir, "lorestead.db");
        ...
        ServerConfig config = new ServerConfig
        {
            ...
            DbKey = dbKey,
            HistoryRetention = ReadClampedInt("LORESTEAD_HISTORY_RETENTION", 50, 10, 100),
            PurgeRetentionDays = ReadClampedInt("LORESTEAD_PURGE_RETENTION_DAYS", 90, 1, 3650),
        };
    ...
    {
        string clientId = Environment.GetEnvironmentVariable("LORESTEAD_OAUTH_CLIENT_ID");
        string clientSecret = Environment.GetEnvironmentVariable("LORESTEAD_OAUTH_CLIENT_SECRET");
        ...
        if (string.IsNullOrWhiteSpace(clientId) != string.IsNullOrWhiteSpace(clientSecret))
        {
            throw new InvalidOperationException(
                "LORESTEAD_OAUTH_CLIENT_ID and LORESTEAD_OAUTH_CLIENT_SECRET must be set together.");
        }

        if (!string.IsNullOrWhiteSpace(clientId))
        {
            string publicUrl = Environment.GetEnvironmentVariable("LORESTEAD_PUBLIC_URL");

            if (string.IsNullOrWhiteSpace(publicUrl))
            {
                throw new InvalidOperationException(
                    "LORESTEAD_PUBLIC_URL is required when OAuth is configured - discovery documents " +
            ...
            string redirectUris = Environment.GetEnvironmentVariable("LORESTEAD_OAUTH_REDIRECT_URIS");
