...
{
    // Turns a directory tree of markdown (a SylvaNote export, an Obsidian vault, or
    ...
    public static class MarkdownImportBuilder
    {
        ...
        private static void AssignIds(List<Node> ordered, Dictionary<string, Note> existingById, HashSet<string> scope, Context context)
        {
            ...
            foreach (Node node in ordered)
            {
                string fmId = node.FrontMatter?.SylvaNoteId;
                string finalId = null;
                Guid parsed;
                ...
                if (!string.IsNullOrEmpty(fmId))
                {
                    if (Guid.TryParse(fmId, out parsed))
                    {
                        ...
                        if (claimed.Contains(fmId))
                        {
                            ...
                            context.Warnings.Add("Duplicate sylvanote-id in \"" + node.File.Path + "\"; imported as a new note.");
                        }
                    ...
                    {
                        context.Warnings.Add("Ignored invalid sylvanote-id in \"" + node.File.Path + "\".");
                    }
                }
