...
{
    // The real Lorestead.Mcp binary over the real stdio transport, pointed at a
    // throwaway data dir via LORESTEAD_DATA_DIR. Spawned by path (not a project
        ...
        public async Task StdioBinaryServesTheFullToolSet()
        {
            ...
            try
            {
                StdioClientTransport transport = new StdioClientTransport(new StdioClientTransportOptions
                {
                    Name = "Lorestead",
                    Command = exe,
                    EnvironmentVariables = new Dictionary<string, string> { ["LORESTEAD_DATA_DIR"] = dataDir },
                });
