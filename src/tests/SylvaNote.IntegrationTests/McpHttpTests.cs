...
namespace SylvaNote.IntegrationTests
{
    ...
    public sealed class McpHttpTests
    {
        ...
        public async Task NoteWritesRoundTripAndEnterTheSyncFeed()
        {
            ...
            List<SylvaNote.Core.Entities.ChangeLogEntry> feed =
                new SylvaNote.Core.DataAccess.ChangeLogRepository(server.ConnectionManager).GetAfter(0, 10);
            SylvaNote.Core.Entities.ChangeLogEntry entry = Assert.Single(feed);
