...
<template>
  <component :is="isButton ? 'button' : 'div'" :type="isButton ? 'button' : undefined"
    ...
    <!-- The label holds its width instead of ellipsizing, so the sidebar's search
         entry slides its text out under the field's edge as the rail collapses
         (the caller clips the box). The input's condition is spelled out rather
         than a v-else, so a label-less button doesn't fall into it. -->
    <span v-if="isButton && label" class="shrink-0 whitespace-nowrap">{{ label }}</span>
